<template>
  <div id="transListMain">
    ...
    <div class="showTransList">
      <el-table ref="approvalTable" :data="approvalListTable" v-loading="loading" style="width: 100%" @row-dblclick="rowDblClick" @sort-change="sortChange" :row-class-name="deadlineHighlight">
        ...
        <el-table-column label="Operation" align="center">
          <template slot-scope="scope">
          <el-button type="text" size="medium" title="Review" @click="transItem(scope.row)" v-if="scope.row.status !== 'Untranslated'">Review</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  mounted() {
    ...
    // !this.$route.query.name&&!this.$route.query.key&&!this.$route.query.priority&&!this.$route.query.deadline?this.searchFlag = false:this.searchFlag = true
      this.$http.post("/api/approve/list",qs.stringify({product_name:this.$route.query.name,key:this.$route.query.key,priority:this.$route.query.priority,deadline:this.$route.query.deadline,status:'Unreviewed',page:this.$route.query.page,count:this.$route.query.count})).then(response=>{
