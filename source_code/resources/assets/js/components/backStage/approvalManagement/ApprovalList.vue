<template>
  <div id="transListMain">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      <el-breadcrumb-item>Approval List</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <div class="showTransList">
      <el-table ref="translationTable" :data="approvalListTable" v-loading="loading" stripe border style="width: 100%" @row-dblclick="rowDblClick">
        ...
        <el-table-column label="Operation" align="center">
          <template slot-scope="scope">
          <el-button type="text" size="medium" icon="el-icon-document" title="view" @click="transItem(scope.row)" v-if="scope.row.status !== 'Untranslated'">View</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    onSearch() {
      ...
      this.$http.post("/api/approve/list",qs.stringify({product_name:this.searchForm.product_name,key:this.searchForm.key,priority:this.searchForm.priority,deadline:this.searchForm.deadline,status:'Unreviewed'})).then(response=>{
        // console.log(response.data);
        this.approvalListTable = response.data.data;
        this.total = response.data.total;
        this.loading = false
    ...
    rowDblClick(row, column, event){
      ...
      // console.log(column);
      // console.log(event);
      this.$router.push({path:'/browapprovalitem',query:{id:row.id}})
    },
    transItem(row) {
      // console.log(row);
    ...
    handleCurrentChange(val) {
      ...
      if(this.searchFlag){
        this.$http.post("/api/approve/list",qs.stringify({product_name:this.searchForm.product_name,key:this.searchForm.key,priority:this.searchForm.priority,deadline:this.searchForm.deadline,status:'Unreviewed',page:val})).then(response=>{
          // console.log(response.data);
          this.approvalListTable = response.data.data;
          this.total = response.data.total;
          this.loading = false
        ...
        this.$http.post("/api/approve/list",qs.stringify({status:'Unreviewed',page:val})).then(response=>{
          // console.log(response.data);
          this.approvalListTable = response.data.data;
          this.total = response.data.total;
          this.loading = false
