  ...
  <div id="projectDetailMain">
    ...
    <div id="projectDetailCon">
              ...
              <el-form-item prop="translate_users" label="Translator">
                <div class="membersInput">
                  ...
                  <el-select v-model="addMembersForm.translate_users" :disabled="!translatorMembersFlag" placeholder="select translator" multiple filterable @visible-change="visibleChange('t',$event)" @change="changeUser('t',$event)">
                    <el-option v-for="(item,index) in currentProject.remainingTranslatorList" :key="'Translator'+index" :label="item.name" :value="item.id"></el-option>
                  </el-select>
              ...
              <el-form-item prop="approve_users" label="Reviewer">
                <div class="membersInput">
                  ...
                  <el-select v-model="addMembersForm.approve_users" :disabled="!approverMembersFlag" placeholder="select reviewer" multiple filterable @visible-change="visibleChange('a',$event)" @change="changeUser('a',$event)">
                    <el-option v-for="(item,index) in currentProject.remainingApproverList" :key="'approver'+index" :label="item.name" :value="item.id"></el-option>
                  </el-select>
              ...
              <el-form-item prop="viewed_users" label="Guest">
                <div class="membersInput">
                  ...
                  <el-select v-model="addMembersForm.viewed_users" :disabled="(currentProject.attribute==='public')||!viewerMembersFlag" placeholder="select guest" multiple filterable @visible-change="visibleChange('v',$event)" @change="changeUser('v',$event)">
                    <el-option v-for="(item,index) in currentProject.remainingViewerList" :key="'Viewer'+index" :label="item.name" :value="item.id"></el-option>
                  </el-select>
        ...
        <el-tab-pane label="Schedule" name="schedule">
          <el-card class="box-card" shadow="always">
            <div slot="header">
              ...
              <el-button class="operationBtn" size="large" type="text" @click="viewIssuesList">Issues</el-button>
            </div>
                ...
                <span>Translator:
                  <div v-for="(item,index) in currentProject.translatorName" :key="index" style="margin:6px 20px;">{{item}} <span style="font-size:14px; font-weight:bold;margin-left:10px">(Total: {{translatorsStatistics[item]?(translatorsStatistics[item].Qualified+translatorsStatistics[item]['Unretranslated']+translatorsStatistics[item].Unreviewed+translatorsStatistics[item].Untranslated):0}})</span>
                    <div>
                      ...
                      <el-progress style="display:inline-block;width:90%" :stroke-width="4" text-inside :percentage="0" v-if='!translatorsStatistics[item]'></el-progress>
                      <span v-if='!translatorsStatistics[item]'>0.00%</span>
                    </div>
                ...
                <span>Reviewer:
                  ...
                  <div v-for="(item,index) in currentProject.approverName" :key="index" style="margin:6px 20px;">{{item}}
                    <div>
                      ...
                      <el-progress style="display:inline-block;width:90%" v-if="JSON.stringify(approversStatistics)=='{}'" :stroke-width="4" text-inside :percentage="0"></el-progress>
                      <span v-if="JSON.stringify(approversStatistics)=='{}'">0.00%</span>
                    </div>
      ...
      <el-dialog title="Upload File" :visible.sync="dialogUploadVisible" @close="closeUploadModal" :close-on-click-modal="false" :close-on-press-escape="false" :show-close="!uploadLoadingFlag">
        ...
        <el-dialog width="70%" title="Upload Completed" :visible.sync="resultVisible" @close="closeResultModal" append-to-body :close-on-click-modal="false" :close-on-press-escape="false">
          <div style="max-height:500px;overflow:auto;">
            <div style="margin-bottom:20px;color:#28a745;">
              <i class="el-message__icon el-icon-success" style="font-size:24px;"></i>
              <span>Counts of uploaded entries:</span>
              <span style="font-weight:bold;">{{uploadResult.success_nums}}</span>
            </div>
            <div v-if="(uploadResult.Duplicate_key.length>0)||(uploadResult.nullkey.length>0)||(uploadResult.overmuch.length>0)">
              <div style="margin-bottom:10px;font-weight:bold;">Reasons for entries failed to upload:</div>
              <div style="margin-left:20px;">
                <div style="display:flex;flex-direction:row;border-top:1px solid #eee;border-bottom:1px solid #eee;">
                  <span style="box-sizing:border-box;padding:5px;font-weight:bold;width:30%;border-left:1px solid #eee;">Reasons</span>
                  <span style="box-sizing:border-box;padding:5px;font-weight:bold;width:70%;border-left:1px solid #eee;border-right:1px solid #eee;">Row ID</span>
                </div>
                <div style="display:flex;flex-direction:row;flex-wrap:wrap;border-bottom:1px solid #eee;" v-if="uploadResult.Duplicate_key.length>0">
                  <span style="box-sizing:border-box;padding:5px;width:30%;overflow-wrap:break-word;border-left:1px solid #eee;">The first column is not unique or other unpredicted error.</span>
                  <span style="box-sizing:border-box;padding:5px;width:70%;overflow-wrap:break-word;border-left:1px solid #eee;border-right:1px solid #eee;">{{uploadResult.Duplicate_key.length>0?uploadResult.Duplicate_key.join(','):''}}</span>
                </div>
                <div style="display:flex;flex-direction:row;flex-wrap:wrap;border-bottom:1px solid #eee;" v-if="uploadResult.nullkey.length>0">
                  <span style="box-sizing:border-box;padding:5px;width:30%;overflow-wrap:break-word;border-left:1px solid #eee;">The first column is null.</span>
                  <span style="box-sizing:border-box;padding:5px;width:70%;overflow-wrap:break-word;border-left:1px solid #eee;border-right:1px solid #eee;">{{uploadResult.nullkey.length>0?uploadResult.nullkey.join(','):''}}</span>
                </div>
                <div style="display:flex;flex-direction:row;flex-wrap:wrap;border-bottom:1px solid #eee;" v-if="uploadResult.overmuch.length>0">
                  <span style="box-sizing:border-box;padding:5px;width:30%;overflow-wrap:break-word;border-left:1px solid #eee;">Inconsistent column counts with the first uploaded file.</span>
                  <span style="box-sizing:border-box;padding:5px;width:70%;overflow-wrap:break-word;border-left:1px solid #eee;border-right:1px solid #eee;">{{uploadResult.overmuch.length>0?uploadResult.overmuch.join(','):''}}</span>
                </div>
              </div>
            </div>
          </div>
          <span slot="footer" class="dialog-footer" v-if="uploadResult.success_nums>0">
            <el-button type="primary" @click="$router.push('/assignment?id='+$route.query.id+'&name='+currentProject.product+'&status=Unassigned&page=1&count=10')">Go to Assign</el-button>
          </span>
        </el-dialog>
      </el-dialog>
    ...
    projectDetail(){
      this.$http.post("/api/product/details",qs.stringify({product_id:this.$route.query.id})).then(response=>{
        ...
        response.data.data.forEach(item=>{
          ...
          this.$set(item,'remainingTranslatorList',[])
          this.$set(item,'remainingApproverList',[])
          this.$set(item,'remainingViewerList',[])
          // console.log(object);
          ...
          this.userList.forEach(item00=>{
            ...
            JSON.parse(item.approve_users).forEach(item22=>{
              // console.log(item11)
              item00.id === item22?flag2 = true:null
            })
            !flag2?item.remainingApproverList.push(item00):null

            JSON.parse(item.viewed_users).forEach(item33=>{
              // console.log(item11)
